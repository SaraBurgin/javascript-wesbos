...
} // Object.keys gives us an array of the colors. Sort is used to put them in order of shortest to longest words.
...
function isValidColor(word) {
  return !!colors[word]; // !! will give us back a boolean of it is true or not
}
...
function handleResult({
  ...
  let color = words.toLowerCase(); // strip any spaces out. Regex matches all spaces globally and replace them with nothing.

  color = color.replace(/\s/g, ''); // check if it is a valid color

  if (!(0, _colors.isValidColor)(color)) return; // if it is, then show the ui for it
...
function start() {
  ...
  const recognition = new SpeechRecognition(); // we need a couple of settings for this
...
start();
colorsEl.innerHTML = displayColors(_colors.colorsByLength); // webkit is refered to as vendor prefixed meaning that the API is not totally done so they will just vendor prefix it by putting their prefix in front of it. Chromes engine used to be called webkit, it's now called blink but there still is a lot of stuff that is called webkit in there.
